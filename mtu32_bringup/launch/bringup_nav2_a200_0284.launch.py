...
ARGUMENTS = [    
    DeclareLaunchArgument(
        'namespace', default_value='a200_0284', description='Top-level namespace'
    ),
    DeclareLaunchArgument(
        'use_namespace',
        default_value='true',
    ...
    DeclareLaunchArgument(
        'params_file',
        default_value= PathJoinSubstitution([get_package_share_directory('mtu32_bringup'), 'config', 'a200', 'nav2.yaml']),
...
def launch_setup(context, *args, **kwargs):
    ...
    mtu_launch_dir = os.path.join(pkg_mtu_bringup, 'launch')
    mtu_config_dir = os.path.join(pkg_mtu_bringup, 'config')
    ...
    return [stdout_linebuf_envvar, bringup_cmd_group]
